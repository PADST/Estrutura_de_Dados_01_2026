...
public class ListaEncadeadaSimples<T> {
	...
	public No<T> get(int index) throws IllegalArgumentException{
		int i = 0;
		if (this.inicio == null)
			throw new IllegalArgumentException ("Nao existe item na lista, vulgo, vou ficar te devendo.");
		...
		if (i < index)
			throw new IllegalArgumentException ("O indice informado nao existe, vulgo, nao to achando.");
		return buffer;
	}
	public int index(T elemento) throws IllegalArgumentException {
		...
		throw new IllegalArgumentException ("Item nao encontrado, vulgo, to achando nao.");
	}
	public void insert (int index, T elemento) throws IllegalArgumentException {
		if (index == 0){
			No<T> buffer_n = new No<>(elemento);
			if(this.inicio != null){
				No<T> buffer_inicio = this.inicio;
				buffer_n.setProximo(buffer_inicio);
				this.inicio = buffer_n;
			}else{
				this.inicio = buffer_n;
			}
		}else{
	...
	public void insert (No<T> item, T elemento) throws IllegalArgumentException{
		...
		item.setProximo(buffer_n);
		buffer_n.setProximo(buffer_proximo);
	}
